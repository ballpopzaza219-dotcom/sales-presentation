...
เป็นบั๊กเก่าที่มีอยู่ก่อนเซสชันนี้ ไม่เกี่ยวกับงานใดๆ ที่ทำในเซสชันนี้เลย) — ดูหัวข้อ ข.9/ข.10 สำหรับส่วนย่อยที่
ทำไปแล้วบางส่วนแต่ยังเหลือ
...
| **ก.4** | **`fx_maker2` เห็นปุ่มอนุมัติเงินสดย่อยทั้งที่ไม่มีสิทธิ์** (พบ 2026-09-12, ยืนยันแล้วว่าเป็นบั๊กเก่าไม่เกี่ยวกับ client_customers/branches/departments — ยังไม่พบ root cause) | **เปิดอยู่ — บล็อก test:regression-all ไม่ให้ EXIT=0 เต็มชุด** |
