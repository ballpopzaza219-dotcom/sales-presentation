...
- ข.14 (แก้แล้ว 2026-09-11, commit `cd0e461`): `tests/attachments-void-cancel.regression.js` เคย hardcode
  ...
  **แก้ไข timeline ย้อนหลัง (2026-09-12)**: ตอนเจออาการนี้ครั้งแรก (CHECK ขาด `branch`/`department` ทั้งที่
  `schema_migrations` บอกว่า apply 0024 สำเร็จ) เคยสรุปสาเหตุผิดว่าเป็น nested-transaction (0023/0024 มี
  `BEGIN;`/`COMMIT;` ฝังอยู่ในไฟล์ ขัดกับที่ `migrate.js` ครอบ transaction ของตัวเองอยู่แล้ว) — แก้โดยลบ
  `BEGIN`/`COMMIT` ออก (commit `b4942a7`) แล้วยืนยันว่า constraint ถูกต้องหลัง apply ใหม่ **แต่รัน full
  suite อีกครั้งแล้วเจออาการเดิมซ้ำ** ทั้งที่บั๊ก nested-transaction แก้ไปแล้วจริง — พิสูจน์ว่าไม่ใช่สาเหตุ
  ที่แท้จริง จึงไล่หาต่อจนเจอ ข.14 (บั๊กนี้) ซึ่งเพียงพอจะอธิบายอาการทั้งสองครั้งได้ครบด้วยตัวเอง (ไฟล์นี้อยู่
  ก่อน `test:branches-departments` ใน chain อยู่แล้วตั้งแต่ก่อนเซสชันนี้) **สรุป**: ข.14 คือสาเหตุที่แท้จริง
  ของอาการนี้ทั้งสองครั้ง ส่วนการลบ `BEGIN`/`COMMIT` ออกจากไฟล์ migration (`b4942a7`) ยังคงเป็นการแก้ที่ถูกต้อง
  ตามหลักการ (ตรงกับ pattern ของ 0001-0022 อยู่แล้ว และป้องกันความเสี่ยงเชิงโครงสร้างจริงที่อาจก่อปัญหาอื่นใน
  อนาคต) แต่ไม่ใช่ตัวที่แก้อาการที่สังเกตเห็นในเหตุการณ์นี้จริง — ไม่ควรเข้าใจผิดว่าเป็นสาเหตุเดียวกันอธิบายซ้ำ
