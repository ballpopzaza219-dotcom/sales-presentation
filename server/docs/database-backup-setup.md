...
- **✅ ทดสอบ live-restore เข้าฐานข้อมูลจริงแบบครบวงจรแล้ว (2026-09-08)**: สร้าง `sitereq_restore_test`,
  `pg_restore` ไฟล์ backup ล่าสุดเข้าไปจริง, เทียบ `count(*)` ของตารางหลัก (`client_journal_entries`,
  `client_payment_vouchers`, `client_purchase_requests`, `customers`, `schema_migrations`) ระหว่าง
  ฐานข้อมูลจริงกับที่ restore มา **ตรงกันทุกตัวเลข** แล้วลบฐานข้อมูลทดสอบทิ้ง — ยืนยันว่า backup ใช้กู้คืนได้
  จริง ไม่ใช่แค่ไฟล์ที่โครงสร้างไม่เสีย
